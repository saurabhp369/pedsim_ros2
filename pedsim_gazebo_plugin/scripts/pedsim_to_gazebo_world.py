...
def generate_pose(x1, y1, x2, y2):
    if x2-x1==0:
        gzb_world.write( "\t  <pose frame=''>{} {}  1.4 0 0 {}</pose>".format( (x2+x1)/2, (y2+y1)/2, 1.57 ))#x1, y1, math.atan((y2-y1)/(x2-x1)) )
    else:
        gzb_world.write( "\t  <pose frame=''>{} {}  1.4 0 0 {}</pose>".format( (x2+x1)/2, (y2+y1)/2, math.atan( (y2-y1)/(x2-x1) ) ) )#x1, y1, math.atan((y2-y1)/(x2-x1)) )

def generate_size(x1, y1, x2, y2):
    l = math.sqrt( pow(y2-y1,2) + pow(x2-x1,2)  )
    gzb_world.write( "\t     <size> {} .2  2.8 </size>".format( l ))
